...
public:
        ...
        if (cn.setConectar()) {
            ...
            if (resultado) {
                ...
                while ((fila = mysql_fetch_row(resultado))) {
                    cout << "------------------- Compras -------------------" << endl;
                    cout << "idCompra: " << fila[0] << ", no de orden de la compra: " << fila[1] << ", id proveedor: " << fila[2] << ", fecha orden: " << fila[3] << ", fecha ingreso: " << fila[4] << endl;

                    int idCompra = atoi(fila[0]); // Obtener el idCompra de la fila actual
                    mostrarDetalles(idCompra, cn); // Pasar el idCompra y la instancia de ConexionBD como argumentos
                    cout << "-----------------------" << endl;
                }
    ...
    void mostrarDetalles(int idCompra, ConexionBD& cn) { // Pasar el idCompra y la instancia de ConexionBD como referencia
        if (cn.setConectar()) {
            string consulta = "SELECT * FROM compras_detalle WHERE idcompra = " + to_string(idCompra);
            const char* c = consulta.c_str();
            mysql_query(cn.setConectar(), c);
            MYSQL_RES* resultado = mysql_store_result(cn.setConectar());
            ...
            if (resultado) {
                MYSQL_ROW fila;
                while ((fila = mysql_fetch_row(resultado))) {
                    cout << "idCompraDetalle: " << fila[0] << ", idProducto: " << fila[1] << ", cantidad:" << fila[2] << ", precio Unitario" << fila[4] << endl;
                }
                mysql_free_result(resultado); // Liberar el resultado de la consulta
            }
            else {
                cout << "Error al consultar la tabla 'compras_detalle': " << mysql_error(cn.setConectar()) << endl;
            }
        }
        else {
            cout << "--- Error en la conexión ---" << endl;
